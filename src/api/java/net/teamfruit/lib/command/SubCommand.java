...
public abstract class SubCommand implements IModCommand {
	...
	@Nullable
	IModCommand getParent() {
		return this.parent;
	}

	@Override
	public @Nullable RootCommand getRoot() {
		final IModCommand command = getRoot0(this);
		if (command instanceof RootCommand)
			return (RootCommand) command;
		return null;
	}

	private @Nonnull IModCommand getRoot0(final @Nonnull IModCommand command) {
		if (command instanceof SubCommand) {
			final IModCommand pcommand = ((SubCommand) command).getParent();
			if (pcommand!=null)
				return getRoot0(pcommand);
		}
		return command;
	}
